// Ken Perlin's "improved noise" reference permutation.
...
/** Multi-octave fBm — smoother, more organic than a single Perlin layer. */
export function fbm2(x: number, y: number, octaves = 3, lacunarity = 2, gain = 0.5): number {
  let sum = 0;
  let amp = 1;
  let freq = 1;
  let norm = 0;
